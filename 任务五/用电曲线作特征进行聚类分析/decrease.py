...
class user_culster():
    ...
    def figure_shuyu(self, receive_data):
        # 进行数据的处理,进行计算传入需要判别的数据和原本数据的距离值
        receive_data_chuli = metrics.cdist_dtw(receive_data, self.data)
        receive_label = self.kmeans.fit(receive_data_chuli)
        return receive_label


data = get_data()

energy_culster = user_culster(data)
